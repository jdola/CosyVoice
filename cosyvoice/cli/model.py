...
class CosyVoiceModel:
    ...
    def llm_job(self, text, prompt_text, llm_prompt_speech_token, llm_embedding, uuid):
        with self.llm_context:
            # Xử lý text như một tensor duy nhất, không phải list các chunk
            if isinstance(text, list):
                text = torch.cat(text, dim=1)
            
            output = self.llm.inference(text=text.to(self.device),
    ...
    def inference(self, text, flow_embedding, llm_embedding=torch.zeros(0, 192),
        ...
        # Không cần thay đổi text thành list các chunk
        ...
        if stream:
            while time.time() - start_time < timeout:
                if self.llm_end_dict[this_uuid] or len(self.tts_speech_token_dict[this_uuid]) > 0:
                    if len(self.tts_speech_token_dict[this_uuid]) >= self.token_min_hop_len + self.token_overlap_len:
                        this_tts_speech_token = torch.concat(self.tts_speech_token_dict[this_uuid][:self.token_min_hop_len + self.token_overlap_len], dim=1)
                        ...
                        with self.lock:
                            self.tts_speech_token_dict[this_uuid] = self.tts_speech_token_dict[this_uuid][self.token_min_hop_len:]
                    if self.llm_end_dict[this_uuid] and len(self.tts_speech_token_dict[this_uuid]) < self.token_min_hop_len + self.token_overlap_len:
                        break
                time.sleep(0.1)
            else:
                logging.warning("LLM processing timed out")
        else:
            p.join(timeout)
            if p.is_alive():
                logging.warning("LLM processing timed out")
